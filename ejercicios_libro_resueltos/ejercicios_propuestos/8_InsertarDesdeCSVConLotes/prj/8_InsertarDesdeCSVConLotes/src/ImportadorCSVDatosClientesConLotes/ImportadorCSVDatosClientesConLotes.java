package ImportadorCSVDatosClientesConLotes;
...
public class ImportadorCSVDatosClientesConLotes {
        ...
        try (BufferedReader fbr = new BufferedReader(new FileReader(nombreFichero));
            ...
            try (PreparedStatement sInsUpd = c.prepareStatement(sqlInsUpd)) {
                ...
                while ((linea = fbr.readLine()) != null) {
                    ...
                    sInsUpd.addBatch();  // en lugar de sInsUpd.executeUpdate();
                    System.out.println("Grabada en base de datos.");
                }
                int[] numSent = sInsUpd.executeBatch();
                c.commit();
                System.out.println(numSent.length+" Sentencias ejecutadas en un lote, cada una de ellas afecta al siguiente número de registros:");
                for(int i=0; i<numSent.length; i++) {
                    System.out.println("Sentencia " + (i+1) + ": " + numSent[i]);
                }
